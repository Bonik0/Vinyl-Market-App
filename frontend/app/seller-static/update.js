...
async function UpdateVinylRecord(id) {
    ...
    try {
        const response = await fetchWithToken(`/api/seller/vinyl-records/${id}`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            currentURL.pathname = `/vinyl-record/${id}`;
...
window.addEventListener('load', function() {
    ...
    setTimeout(() => {
        fetchWithToken(this.window.location.href + '-info', {
            method: 'GET'
        }).then(response => {
                if (!response.ok) {
                    const currentUrl = new URL(window.location.href); 
